/**
 * Validate an externally issued production approval envelope against the exact
 * release candidate. This module never creates approval and never elevates
 * authority; the independent verifier remains an injected protected boundary.
 *
...
export function evaluateProductionApprovalEnvelope(
  ...
  const reasons = [];
  const approvedRevision = typeof envelope.approvedRevision === "string" ? envelope.approvedRevision.trim() : "";
  const approvedDigest = typeof envelope.approvedDigest === "string" ? envelope.approvedDigest.trim() : "";
  ...
  if (Number.isFinite(issuedAt) && Number.isFinite(expiresAt) && issuedAt >= expiresAt) reasons.push("APPROVAL_WINDOW_INVALID");
