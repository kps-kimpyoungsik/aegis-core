...
export function validateControlPlane({ ownership, domains, capabilities, ledger }) {
  ...
  for (const capability of capabilities.capabilities) {
    if (capabilityIds.has(capability.id)) throw new Error(`AEGIS-WS-003 DUPLICATE_CAPABILITY ${capability.id}`);
  ...
  for (let i = 0; i < active.length; i += 1) {
    for (let j = i + 1; j < active.length; j += 1) {
      ...
      const sharedCapabilities = (left.capabilityIds ?? []).filter((id) => (right.capabilityIds ?? []).includes(id));
      if (sharedCapabilities.length > 0) {
        throw new Error(`AEGIS-WS-010 ACTIVE_CAPABILITY_COLLISION ${left.id}:${right.id}:${sharedCapabilities.join(",")}`);
      }
