...
const RELEASE_EVIDENCE_PREFIX = "product/release/";
...
export function shouldEnforceManifestBaseline(changedPaths) {
  return changedPaths.some((path) => path.startsWith(RELEASE_EVIDENCE_PREFIX));
}
...
export function runPreflight({ expectedBaseSha, manifestPath, changedPaths, addedPaths, basePaths }) {
  const manifestBaselineEnforced = shouldEnforceManifestBaseline(changedPaths);
  if (manifestBaselineEnforced) {
