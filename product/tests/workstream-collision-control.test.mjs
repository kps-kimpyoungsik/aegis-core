...
function fixture() {
  return {
    ...
    ledger: { workstreams: [
      ...
      { id: "ws-b", state: "REFERENCE", ownerResponsibility: "owner-a", capabilityIds: ["cap-a"], touchPaths: ["a/"] },
    ] },
  };
}

test("valid control plane allows reference workstream to overlap active implementation", () => {
...
test("workstream cannot claim a capability owned by another canonical responsibility", () => {
  const data = fixture();
