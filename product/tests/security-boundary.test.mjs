...
const auth = (principal) => async () => principal;
...
test("role mismatch rejects task execution and does not invoke application", async () => {
  let calls = 0;
  await withServer({ executeTask: async () => { calls += 1; return { status: "ACCEPTED" }; }, authenticateRequest: auth(viewer) }, async (port) => {
...
test("wrong content type is rejected for authenticated task command", async () => {
  await withServer({ executeTask: async () => ({ status: "ACCEPTED" }), authenticateRequest: auth(executor) }, async (port) => {
    const res = await request(port, { method: "POST", path: "/v1/tasks", headers: { "x-aegis-tenant": "tenant-a", "content-type": "text/plain" }, body: "{}" });
...
test("malformed json is rejected without internal detail leakage", async () => {
  await withServer({ executeTask: async () => ({ status: "ACCEPTED" }), authenticateRequest: auth(executor) }, async (port) => {
    const res = await request(port, { method: "POST", path: "/v1/tasks", headers: { "x-aegis-tenant": "tenant-a", "content-type": "application/json" }, body: "{" });
...
test("declared oversized request is rejected before body parsing", async () => {
  await withServer({ executeTask: async () => ({ status: "ACCEPTED" }), authenticateRequest: auth(executor) }, async (port) => {
    const res = await request(port, {
      method: "POST",
      path: "/v1/tasks",
      headers: { "x-aegis-tenant": "tenant-a", "content-type": "application/json", "content-length": String(64 * 1024 + 1) },
    });
