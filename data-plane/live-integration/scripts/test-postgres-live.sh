...
BEGIN
  ...
  IF rc <> 1 OR oc <> 1 THEN RAISE EXCEPTION 'source/outbox atomicity verification failed'; END IF;
END $$;
...
DECLARE v bigint;
BEGIN
  SELECT version INTO v FROM aegis_record WHERE tenant_id='legacy' AND dataset_id='runtime.execution' AND record_id='exec-1';
